...
- `prune_dashboard_snapshots.py` plans or applies bounded deletion of old dashboard read-model snapshot metadata while preserving `latest.json`, schemas, index rows, Layer 1/2 data, and SQL.
- `refresh_historical_task_progress_read_model.py` runs the manager-owned `historical_task_progress_summary` semantic producer and materializes the validated payload under `storage/dashboard_cache/`.
