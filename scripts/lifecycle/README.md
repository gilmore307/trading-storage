...
- `verify_single_file_compression_restore.py` verifies single-file zstd compressed copies by decompression and checksum comparison without restoring originals or mutating indexes.
...
- `execute_rerun_reset_lifecycle.py` plans or applies storage-owned filesystem cleanup for a manager model-group rerun reset. Dry-run is default; `--apply --approval-ref <ref>` deletes only unprotected scope-matched generated candidates, writes a reset lifecycle receipt, appends tombstones, and reports unmatched provider-task refs or blocked SQL classes.
- `run_storage_maintenance.py` runs the storage-owned maintenance wrapper, inventories the numbered storage roots, monitors completed manager folds when `--manager-root` is provided, reports lifecycle gaps by default, and writes `storage_scheduled_maintenance_summary`. With explicit `--apply-lifecycle-gap-actions`, it writes compact contracts and applies the reviewed state-triggered delete/compress/rolling actions for replay verbose rows, post-replay attribution rows, failure-triage rows, TE recent-refresh manifests, and normal completed realtime monitor loops.
...
- `consolidate_trading_economics_source.py` consolidates Trading Economics CSV source rows from old monthly/realtime/replay roots into the canonical month-bucketed `storage/01_source_data/monthly_backfill/trading_economics_calendar_web/YYYY-MM/runs/<run_id>/` layout, moves originals under a `_manifests/source_consolidation_*` archive inside that canonical root, and records rejected date-only/unparseable rows without deleting TE payloads.
