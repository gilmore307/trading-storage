...
- `run_storage_maintenance.py` runs the scheduled maintenance wrapper for storage-owned timed cleanup phases, inventories the numbered storage roots, monitors completed manager folds when `--manager-root` is provided, and writes `storage_scheduled_maintenance_summary`.
