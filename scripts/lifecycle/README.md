...
- `execute_rerun_reset_lifecycle.py` plans or applies storage-owned filesystem cleanup for a manager model-group rerun reset. Dry-run is default; `--apply` deletes only unprotected generated candidates, writes a reset lifecycle receipt, appends tombstones, and reports blocked provider-task or SQL classes.
