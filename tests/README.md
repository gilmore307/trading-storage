...
- `test_artifact_index.py` verifies conservative filesystem artifact indexing, Layer 1/2 foundation versus runtime/log classification, explicit lifecycle metadata classification, explicit dashboard read-model indexing, and JSONL/summary writes.
...
- `test_lifecycle_planner.py` verifies durable-artifact dry-run lifecycle planning, protected retention, compression/quarantine/archive candidates, fold-complete source cleanup classification, evidence retention, policy loading, and JSON output round trips.
