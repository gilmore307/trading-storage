...
- `test_artifact_index.py` verifies conservative filesystem artifact indexing, Layer 1/2 foundation versus runtime/log classification, replay summary/input/download classification, explicit lifecycle metadata classification, explicit dashboard read-model indexing, and JSONL/summary writes.
...
- `test_lifecycle_planner.py` verifies durable-artifact dry-run lifecycle planning, protected retention, compression/quarantine/archive candidates, fold-complete source cleanup classification, replay summary/download planning, evidence retention, policy loading, and JSON output round trips.
- `test_protected_set.py` verifies conservative protected-set blocking, replay result summary protection, manual pins, reason-code reference matching, clear-candidate reporting, and JSON output round trips.
...
- `test_dashboard_read_models.py` verifies common dashboard read-model envelope validation, unsafe payload rejection, state-change snapshot materialization, and latest-only refresh behavior.
...
- `test_dashboard_snapshot_lifecycle.py` verifies dashboard snapshot metadata recent-count retention, dry-run/apply pruning, latest preservation, Layer 1/2/SQL non-mutation summary flags, and plan/summary output writes.
