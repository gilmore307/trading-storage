...
- `test_artifact_index.py` verifies conservative filesystem artifact indexing, M01/M02 foundation versus runtime/log classification, replay summary/input/download classification, semantic event interpretation retention versus raw event/news sidecar cleanup, run-local proof sidecar TTL classification, explicit lifecycle metadata classification, explicit dashboard read-model indexing, and JSONL/summary writes.
