...
- `test_artifact_index.py` verifies conservative filesystem artifact indexing, M01/M02 foundation versus runtime/log classification, replay summary/input/download classification, semantic event interpretation retention versus raw event/news sidecar cleanup, run-local proof sidecar TTL classification, dashboard-active input protection, explicit lifecycle metadata classification, explicit dashboard read-model indexing, and JSONL/summary writes.
...
- `test_lifecycle_planner.py` verifies durable-artifact dry-run lifecycle planning, protected retention, compression/quarantine/archive candidates, fold-complete source cleanup classification, replay summary/download planning, raw event/news sidecar cleanup planning, boundary evidence retention versus TTL receipt quarantine, policy loading, and JSON output round trips.
- `test_protected_set.py` verifies conservative protected-set blocking, replay result summary, semantic event interpretation and dashboard-active input protection, manual pins, reason-code reference matching, clear-candidate reporting, and JSON output round trips.
