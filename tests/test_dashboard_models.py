            ...
            layer_rows.append(
                {
                    ...
                    "chosen_action": f"{layer_id}:accept_diagnostic_state"
                    if point_in_time_diagnostic
                    else "open_long",
                    "available_action": [f"{layer_id}:accept_diagnostic_state", f"{layer_id}:downweight_or_reject"]
                    if point_in_time_diagnostic
                    else ["open_long", "baseline_action"],
                    "best_available_action_by_future_outcome": "baseline_action" if incorrect else (
                        f"{layer_id}:accept_diagnostic_state" if point_in_time_diagnostic else "open_long"
                    ),
                    "chosen_action_return": None if point_in_time_diagnostic else -0.08 if incorrect else 0.04,
                    "best_available_action_return": None if point_in_time_diagnostic else 0.0 if incorrect else 0.04,
    ...
    (layer_root / "operation_component_metrics.csv").write_text(
        "\n".join(
            [
                ...
                "3,C03_lifecycle_operation,component_03_lifecycle,C03 Lifecycle,portfolio_lifecycle_state,held_position_continuity_and_final_state,portfolio_lifecycle_state,computed,,portfolio_selection_summary;continued_held_position_count;final_position_count,,2,1,0,,1,,,,,,,1.0,True,False,False,True",
...
class DashboardModelsTests(unittest.TestCase):
    ...
    def test_replay_review_summary_projects_review_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(c03["applicability_status"], "portfolio_lifecycle_state_reviewed")
            self.assertEqual(c03["input_count"], 2)
            self.assertEqual(c03["computed_metric_count"], 1)
            self.assertEqual(c03["not_applicable_metric_count"], 0)
            ...
            self.assertEqual(replay_operations["component_action_rows"][0]["trigger_state"], "clock_triggered")
            self.assertEqual(
