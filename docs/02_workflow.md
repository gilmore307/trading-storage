...
For historical data acquisition, development outputs first live under `trading-data/data/storage/` and are not owned as durable storage. Draft receipt/output shapes are in `trading-main/templates/data_tasks/completion_receipt.json` and `templates/data_tasks/save_spec.md`. `trading-storage` should eventually provide the SQL table/partition contract named by `trading-data` task key files and the durable location/schema for data task completion receipts.
