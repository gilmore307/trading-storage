...
- Implement the next storage lifecycle phase in controlled slices: file compression executor, SQL archive executor, restore verifier, and finally a daemon/scheduled maintenance wrapper.
- Keep lifecycle execution dry-run-only until lifecycle mutation executors and receipts are implemented and reviewed. The V0.1 filesystem artifact index, protected-set builder, durable-artifact lifecycle planner, and quarantine/recheck evidence builder are implemented for inventory/safety/planning/evidence only; they do not authorize production mutation.
