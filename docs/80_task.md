...
- Accepted the durable non-SQL data rule: system-owned non-SQL saved data belongs in `trading-storage` by default, while semantic ownership stays with the producing component. Extended the boundary so trading runtime disposable cache/tmp/local staging also belongs in storage-owned ignored roots with storage-owned scheduled cleanup; component-local staging is transitional only.
