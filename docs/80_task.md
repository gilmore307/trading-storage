...
- Implement the next storage lifecycle phase in controlled slices: dry-run lifecycle planner, file compression executor, SQL archive executor, restore verifier, and finally a daemon/scheduled maintenance wrapper.
- Keep lifecycle execution dry-run-only until quarantine-before-delete and lifecycle receipts are implemented and reviewed. The V0.1 filesystem artifact index and protected-set builder are implemented for inventory/safety evidence only; they do not authorize production mutation.
