...
The storage-owned reset executor is `scripts/lifecycle/execute_rerun_reset_lifecycle.py`. It consumes a manager `model_group_rerun_plan` or a manager reset result containing a `plan` field. Dry-run is the default and writes a receipt with concrete candidates, retained paths, blocked classes, unmatched refs, and dashboard refresh requirements. `--apply` requires an `--approval-ref`, deletes only unprotected filesystem candidates, writes a `storage_rerun_reset_lifecycle_receipt`, and appends tombstones under `storage/90_lifecycle/tombstones/rerun_reset_lifecycle/`.
...
| `stage_receipts` | `storage/02_control_plane/runtime/model_training_stage_receipts/<stage_key>/*.receipt.json` | Delete only files in downstream stage buckets whose path/content matches the accepted reset scope; unmatched shared buckets are reported instead of deleting the whole directory. |
| `stage_logs` | `storage/02_control_plane/runtime/model_training_stage_logs/<stage_key>/*.stdout.log`, `*.stderr.log` | Delete only scope-matched files as runtime sidecars; do not remove a shared stage-key bucket wholesale. |
| `task_progress_sidecars` | `storage/02_control_plane/runtime/task_progress/*<stage_key>*` | Delete downstream progress files only when path/content matches the reset scope. |
| `provider_task_sidecars` | `storage/02_control_plane/runtime/provider_task_keys/**/task_key.json` | Delete task-key directories/files only when path/content matches the accepted reset target and window/candidate scope. Unmatched provider keys are reported instead of guessed. |
...
- `active_consumer_input`: source/runtime/control-plane files currently read by dashboard/read-model refresh, scheduler summaries, repair, or model readiness. Keep until the consumer is migrated to a compact/latest read model, bounded tail, SQL row, or artifact-index route and a readback test proves equivalent behavior.
...
- dashboard-active source/runtime inputs: retain while dashboard producers read them directly. Scheduler decision logs, scheduler state, active workflow state, stage coverage, stage-run dashboard inputs, execution runtime status, latest/current-window realtime monitor artifacts, and current TE freshness inputs can roll only after compact summaries/latest pointers or bounded tails replace the direct path.
