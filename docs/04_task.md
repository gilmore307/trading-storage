...
| --- | --- | --- | --- | --- |
| P0 | `storage/06_dashboard_cache/**/snapshots/` | Timestamped dashboard snapshots previously grew without bound; cache is now latest-only and about 296K with zero timestamped snapshots. | Keep `latest.json` and schema only. Scheduled refresh should continue auto-pruning timestamped read-model snapshots. | Current route implemented; monitor for regression. |
| P0 | `storage/01_source_data/monthly_backfill/alpaca_bars/<symbol>/<YYYY-MM>/` | About 13G total. Duplicate payload classes include about 9.7G `equity_bar.jsonl` and about 3.5G `equity_bar.csv` across the same symbol-month runs. | Verify SQL/canonical row coverage, then replace duplicate raw file payload retention with compact provenance: provider, symbol, month, row count, hashes, and source receipt. Preserve unrecoverable source evidence until the verification gate proves a safe compression or removal route. | Add SQL coverage check and directory-level compression/removal plan. |
| P0 | `storage/02_control_plane/runtime/layer_09_option_expression/**/task_key.json` | About 82k task-key files; Layer 9 runtime is about 759M on disk. These are generated control-plane metadata, not durable market source data. | Replace per-request task-key sprawl with compact batch manifests plus accepted receipts. TTL or quarantine old per-request keys after provider tasks complete and gate receipts remain available. | Add lifecycle selector and producer retention metadata. |
| P0 | `storage/02_control_plane/runtime/layer_09_option_expression/gate_review/*.json` | One 2016-01 gate review is about 103M. Gate decisions are important evidence but should not become full verbose runtime dumps. | Preserve compact gate decision, blocker, repair, retry, and receipt references. Move verbose diagnostic payloads to bounded debug retention or compression. | Add compact gate-review contract before pruning. |
...
| P1 | `storage/01_source_data/monthly_backfill/trading_economics_calendar_web/YYYY-MM/runs/calendar_maintenance_*_te/` | TE source is protected and canonical under monthly backfill, but 2026-06 and 2026-07 refresh runs are accumulating. | Preserve TE source facts. Verify refresh runs are incremental/deduplicated and compact repeated run evidence into concise provenance. Do not delete TE source payloads without TE-specific lifecycle review. | Add TE duplicate-run audit. |
...
| P2 | `storage/01_source_data/layer_09_option_expression/` | Current size is modest, about 85M, but future full option-chain ingestion can dominate storage. | Store shared option source once for Layer 3 and Layer 9 reuse. Keep SQL/canonical source plus compact provenance; avoid layer-local duplicate payload copies. | Coordinate with option-source contract work. |
