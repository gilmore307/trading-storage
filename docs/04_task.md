...
| P1 | `storage/01_source_data/monthly_backfill/trading_economics_calendar_web/YYYY-MM/runs/{calendar_maintenance_*_te,te_recent_calendar_refresh_*}/` | TE source rows/payloads are protected and canonical under monthly backfill. Audit on 2026-06-13 found 655 `calendar_maintenance_*_te` run directories and about 837M across 2026-06/2026-07. Calendar maintenance now runs TE discovery daily, writes only new or changed source facts, cleans up no-op refreshes without receipts/byproducts, schedules one-shot release jobs at discovered future release times, polls TE every 5 seconds for up to 60 seconds for formal `actual` values, and writes provisional web-search fallback evidence only when TE is late. TE side products are not protected data once compact provenance/reconciliation exists. | Preserve TE source rows/payloads. Add lifecycle selectors for duplicate receipts/manifests, diagnostics, no-op context, and provisional fallback artifacts after formal TE capture. Compact repeated run evidence into concise month-level provenance; do not delete TE source rows/payloads. | Growth bounded by changed facts and release-time fetches; next slice is side-product selector/reconciliation, not TE data deletion. |
