...
- Keep broad/destructive lifecycle execution unauthorized. Current allowed writes are compressed-copy creation for unprotected `compress_candidate` regular files and gzip archive-copy creation for unprotected file-backed `archive_candidate` records. Dashboard snapshot deletion is on hold until event-risk-governor regeneration and downstream review close.
...
- Maintain storage-hosted dashboard read models for the current public set: `current_system_status_summary`, `historical_task_progress_summary`, `realtime_signal_summary`, and `execution_realtime_trading_runtime_status`.
