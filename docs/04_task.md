...
| P1 | `storage/01_source_data/monthly_backfill/trading_economics_calendar_web/YYYY-MM/runs/{calendar_maintenance_*_te,te_recent_calendar_refresh_*}/` | TE source is protected and canonical under monthly backfill. Audit on 2026-06-13 found 655 `calendar_maintenance_*_te` run directories and about 837M across 2026-06/2026-07. Calendar maintenance now runs TE discovery daily, writes only new or changed source facts, cleans up no-op refreshes without receipts/byproducts, and schedules one-shot release fetches just after discovered future release times; `te_recent_calendar_refresh_*` remains legacy protected evidence, not a disposable temp name. | Preserve TE source facts. Verify refresh runs are incremental/deduplicated and compact repeated run evidence into concise provenance. Do not delete TE source payloads without TE-specific lifecycle review, protected-set clearance, and restore/readback evidence. | Growth bounded by changed facts and release-time fetches; cleanup deferred pending TE-specific lifecycle decision evidence. |
