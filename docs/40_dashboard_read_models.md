...
`trading_storage.dashboard_models` produces `model_readiness_summary` and `model_promotion_posture_summary` for the dashboard Models page. These summaries are derived from `historical_task_progress_summary`, `execution_realtime_trading_runtime_status`, and local promotion-review artifacts; they do not query raw model tables, activate models, place orders, or mutate lifecycle state. The model-group portion owns promotion-version history, active/shadow/retired identity, AUROC/return/drawdown/PCA metric fields, agent recommendation, and decision status because promotion is decided for the whole pipeline. Model-group replay evidence is publishable only when the replay receipt proves a M02 target-candidate handoff or an explicit reviewed preview override; M01/M02 base-context refs are not trade candidates and do not make a replay eligible for display. Promotion evidence is also excluded when any required evidence-chain artifact predates the latest `manager_model_group_rerun_reset_receipt` for the same target/fold window, so rerun reset receipts invalidate stale dashboard rows without deleting audit evidence. Model readiness rows provide model family, model role, optimization target, and component version refs for group-version context only. Task state and workflow blockers remain Tasks/Diagnostics concerns rather than model-page validity labels.

### Replay review producer

`trading_storage.dashboard_replay_review` produces `model_group_replay_review_summary` for Replay Performance, Replay Decisions, Replay Operations, and Replay Attribution. It reads completed post-replay review artifacts under `storage/05_replay_datasets/promotion_replay_candidate_policy/post_replay_review_runs`, the replay execution `decision_rows_ref` named by the matching review receipt, plus M03 event-attribution artifacts under `post_replay_attribution_runs`, and publishes only the latest completed post-replay review run for each current `candidate_fold_id`. The payload is a compact projection: review run identity, performance summary, replay review row aggregates, standard replay diagnostics, M01-M05 effective layer traces with scored correctness overlays where published, M03 interpreted event/effect disposition counts, C01-C07 operation component summaries and concrete action rows from `operation_component_action_rows.csv`, component metric rows from `operation_component_flow.csv`, `operation_component_review_packet.csv`, and `operation_component_metrics.csv`, parameter replay review classes, event proposal/status counts, focus samples, and source refs. Model Groups does not consume this read model as a primary surface; it stays limited to machine-learning model quality, calibration, feature-space, data-integrity, temporal-stability, readiness, and promotion/shadow posture.

Standard replay diagnostics are first-class read-model evidence, not dashboard-only recalculation. Each review run carries `standard_review_diagnostics` with candidate-entry funnel counts/rates, pre-option candidate quality, M05 expression mechanics, DTE/hard-filter summaries, and operation mechanism-contract status. The chart payload also carries `cross_model_group_diagnostics`, which detects duplicate normalized concrete-operation traces across replayed groups and flags operation-path independence risk for Replay Operations or Replay Attribution. Replay Performance consumes economic context; Replay Decisions consumes M01-M05 probability-surface and layer-decision context; Replay Operations consumes C01-C07 component-specific operation context. Model Groups must not expose replay returns, operation traces, component action rows, or attribution verdicts as primary model-page content.

Replay Decisions and Replay Operations share selector and drilldown structure, but not one metric method: M01-M05 rows carry layer-specific `metric_family`, `analysis_method`, label provenance, review-boundary reference/status, upstream-state isolation policy, downstream-input policy, responsibility assignment policy, factor ownership policy, and fusion responsibility policy, while C01-C07 operation rows carry component-specific `operation_action`, `operation_status`, input/output summaries, block reason, `analysis_method`, `evidence_role`, `label_role`, trigger state, point-in-time feasible action-set reference/count/status, review-boundary reference/status, upstream-state isolation policy, downstream-input policy, responsibility assignment policy, component objective, chosen action, best available post-replay label action, chosen/best return, ex-post rank when known, component correctness class, label basis, and optional post-replay outcome values. Replay Decisions may plot actual movement against a model probability interval only when the layer rows publish comparable actual path/return fields plus lower/upper distribution bounds; it must not treat scalar scores as price or return intervals. Replay Operations may plot operation marks on the replay K-line and use marker clicks to filter C01-C07 operation rows. M01 rows are targetless replay-time background states; M02 rows are sourced from the full point-in-time model-candidate trace and are not filtered by later selected trades; M03 rows are sourced from the point-in-time interpreted event/effect ledger with one row per event effect, never from M02 candidate expansion or downstream selected trades. M03 summary rows publish event-impact disposition counts for no impact, context only, risk shape, directional effect, and unresolved rows. M01-M03 must each have an independent post-replay label join before their local factor output can be called acceptable; a missing M01/M02/M03 label is an upstream review evidence gap, not proof that M04 is responsible. When M01, M02, and M03 are independently acceptable but final selected performance is poor, Replay Decisions should attribute the first model-layer defect to M04 fusion, weighting, calibration, thresholding, or action timing unless an explicit interface/handoff defect is shown earlier. M04-M05 rows are sourced only from decisions that reached the underlying-action or expression layer and can carry post-replay action/expression outcome labels. Layer decision rows are exposed through paginated detail reads instead of truncating each layer to the first visible page. C03 lifecycle is part of the operation chain and must publish portfolio lifecycle/replacement evidence or an explicit lifecycle evidence gap, not a non-applicable replay-mode marker. C04 expression review covers equity options-first expression, option rolling/replacement, crypto leverage expression, and direct-underlying fallback eligibility; C05/C06 own order intent and execution gates. Future facts are review labels only and may rank only actions visible and feasible at decision time. Downstream layer/component rows are judged only against the upstream state and candidate set they actually received; upstream defects are attributed to the earliest layer or boundary where they became knowable, and interface/handoff defects remain explicit boundary failures. The producer performs no replay, provider calls, model activation, broker execution, account mutation, or source artifact mutation.
