...
Replay Decisions and Replay Operations share selector and drilldown structure, but not one metric method: M01-M05 rows carry layer-specific `metric_family`, `analysis_method`, and label provenance, while C01-C07 operation rows carry component-specific `operation_action`, `operation_status`, input/output summaries, block reason, `analysis_method`, `evidence_role`, `label_role`, and optional post-replay outcome values. C03 lifecycle is part of the operation chain and must publish portfolio lifecycle/replacement evidence or an explicit lifecycle evidence gap, not a non-applicable replay-mode marker. The producer performs no replay, provider calls, model activation, broker execution, account mutation, or source artifact mutation.
