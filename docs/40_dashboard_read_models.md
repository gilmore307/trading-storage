...
- `src/trading_storage/dashboard_models.py` builds and materializes `model_layer_readiness_summary` and `model_promotion_posture_summary` from already-materialized task-progress and execution-runtime summaries. It reports model-group active pointer evidence, shadow/retiring/eliminated posture, promotion/evaluation evidence, and layer candidate refs for the Models page.
...
`trading_storage.dashboard_models` produces `model_layer_readiness_summary` and `model_promotion_posture_summary` for the dashboard Models page. These summaries are derived from `historical_task_progress_summary` and `execution_realtime_trading_runtime_status`; they do not query raw model tables, activate models, place orders, or mutate lifecycle state. The model-group portion owns active live pointer, shadow/retiring/eliminated refs, evaluation evidence, promotion evidence, and promotion blockers because promotion is decided for the whole pipeline. Layer rows provide component candidate refs and update evidence only. Task state and workflow blockers remain Tasks/Diagnostics concerns rather than model-page status labels.
