...
- `src/trading_storage/dashboard_models.py` builds and materializes `model_layer_readiness_summary` and `model_promotion_posture_summary` from already-materialized task-progress and execution-runtime summaries. It reports registered candidate versions, evaluation/promotion posture, active pointer evidence, shadow/retiring/eliminated posture, and blockers for the Models page.
...
`trading_storage.dashboard_models` produces `model_layer_readiness_summary` and `model_promotion_posture_summary` for the dashboard Models page. These summaries are derived from `historical_task_progress_summary` and `execution_realtime_trading_runtime_status`; they do not query raw model tables, activate models, place orders, or mutate lifecycle state. The layer summary owns the owner-facing model page shape: one row per Layer 1-10 model, observed candidate versions, active live pointer, shadow/retiring/eliminated refs, evaluation evidence, promotion evidence, blockers, and training/workflow evidence lineage. The promotion summary owns compact per-layer promotion posture rows.
