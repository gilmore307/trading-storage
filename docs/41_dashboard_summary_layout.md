...
- `read_models/<contract_type>.json` is the newest accepted summary for fast dashboard reads.
...
| Model readiness, model evidence requirements, and model metrics | `trading-model` |
...
Implementation must validate each summary before the current read-model file is replaced.
...
7. Current read-model file update is atomic after validation succeeds.
...
- retain one current read-model file for every registered summary contract;
...
- `src/trading_storage/dashboard_models.py` and `scripts/dashboard/refresh_public_dashboard_read_models.py` build and materialize the Models-page set: `model_readiness_summary` and `model_promotion_posture_summary`.
...
- `temporal_explorer_summary` for the Events attention-pool page;
...
- `model_readiness_summary` and `model_promotion_posture_summary` for the Models page.
