...
## D025 - Dashboard summaries retain latest only
...
Retain `latest.json` and schema files for dashboard read models. `latest.json` is updated on every accepted refresh. Timestamped full snapshots are not generated by the current read-model writer.
...
- `latest.json`, schemas, Layer 1/2 data, SQL data, and canonical evidence are not deletion targets for the dashboard snapshot pruner.
