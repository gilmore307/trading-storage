...
Date: 2026-05-12
Status: Superseded by D025 compact current-file layout
...
Accept `docs/41_dashboard_summary_layout.md` as the first physical layout and validation-boundary contract for dashboard summaries. That snapshot-era physical layout has been superseded by the compact current-file route.

The current compact layout keeps only:
...
- `trading-dashboard` should read storage-hosted current summaries, not raw manager/model/data/execution/storage internals.
...
## D025 - Dashboard summaries retain one current file only
...
`storage/06_dashboard_cache` is a read-model cache, not the canonical evidence store. Keeping refreshes as full timestamped dashboard snapshots grows unbounded and duplicates the current dashboard summary. Historical chart needs should use compact metric series, not repeated full read-model JSON.

### Decision

Retain one current `<contract_type>.json` file and one schema file for each dashboard read model. The current file is updated on every accepted refresh. Timestamped full snapshots and materialization index rows are not generated by the current read-model writer.
...
use latest-only retention. The default prune plan keeps zero timestamped snapshots per contract and marks timestamped dashboard snapshots as delete candidates. The optional age grace flag may be used for short debugging windows, but it is not the default retention mechanism.
