...
| Benchmark datasets | `storage/05_benchmark_datasets/` | `trading-evaluation` with storage-owned placement | Frozen benchmark preparation bundles, replay dataset manifests, acquisition plans, reusable Layer 1/2 and event/news benchmark inputs, model-pipeline benchmark result summaries, and model-specific temporary benchmark downloads. |
