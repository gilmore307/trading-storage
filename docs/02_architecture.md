...
| Repository-local temporary scratch | `tmp/` | local helper | Deleted after 3 days. Not archived. |
| Repository-local logs | `logs/` | local helper | Archived after 14 days, then active copy is removed. |
| Repository-local run staging | `runs/` | local helper | Archived after 30 days, then active copy is removed. |
| Repository-local development outputs | `outputs/` | local helper | Archived after 30 days, then active copy is removed. |
...
All runtime classes above are ignored by Git. The accepted trading runtime route is the storage-owned ignored roots under `storage/90_lifecycle/`; repository-local `tmp/`, `runs/`, `outputs/`, logs, and cache paths remain covered by the same cleanup policy when present.
