...
- For historical `trading-data` tasks, development outputs/receipts live under `trading-data/storage/` and are disposable. Storage should own later SQL output destination contracts and durable completion receipt storage/reference formats; it must not own provider semantics or manager lifecycle.
