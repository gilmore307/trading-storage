...
- For historical `trading-source` tasks, development outputs/receipts live under `trading-source/storage/` and are disposable. Storage should own later SQL output destination contracts and durable completion receipt storage/reference formats; it must not own provider semantics or manager lifecycle.
