...
| --- | --- | --- | --- | --- |
| `storage/06_dashboard_cache/read_models/<contract>/latest.json` | Current materialized dashboard read model for one contract. | Serves the dashboard current state without querying every producer. | `trading-dashboard` read-model API, dashboard refresh scripts, `dashboard_system_status`, `dashboard_models`, `dashboard_temporal_explorer`, tests. | Do not delete while the contract is public. It is replaced atomically by refresh. |
| `storage/06_dashboard_cache/schemas/<contract>.schema.json` | Schema placeholder for a dashboard read-model contract. | Validates and documents the current read-model envelope. | Dashboard materializer, artifact index, tests, docs. | Do not delete while the matching `latest.json` exists. |
| `storage/06_dashboard_cache/index/dashboard_read_model_index.jsonl` | Compact read-model materialization index. | Records latest materialization events and supports artifact indexing. | Dashboard snapshot lifecycle helper, artifact index, tests. | Compact with `compact_dashboard_read_model_index`; keep bounded latest index evidence. |
| `storage/06_dashboard_cache/read_models/<contract>/snapshots/**/*.json` | Old full dashboard read-model snapshots. | Former state-change history for dashboard payloads. | No current required consumer; current dashboard uses `latest.json`. | Delete by dashboard snapshot lifecycle pruning. Keep zero timestamped snapshots unless a debugging grace window is explicitly approved. |
...
3. Confirm the current consumer has a replacement: `latest.json`, compact summary, batch manifest, SQL rows, source receipt, lineage ref, or archive.
