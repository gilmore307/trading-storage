...
| `storage/01_source_data/monthly_backfill/trading_economics_calendar_web/YYYY-MM/runs/{calendar_maintenance_*_te,te_recent_calendar_refresh_*}/` side products | Run-local byproducts around TE source rows: duplicate request manifests, duplicate completion receipt snapshots, diagnostics, empty/no-op context, and repeated provenance fragments after compact month-level provenance exists. | Operational trace/debug evidence; not the TE data itself. | Storage lifecycle audit, repair/debug during a short retention window. | Compact to month-level provenance plus hashes, then delete or compress duplicates by regular lifecycle cadence. Keep one concise provenance trail and do not remove source row payloads. |
...
| `storage/01_source_data/model_05_option_expression/` | Option-expression source payload area. | Current M05 option data source evidence; future shared option source for Layer 3/M05. | M05 data acquisition/replay; planned Layer 3 target-state option features. | Do not create layer-local duplicate stores. Normalize into one shared option source/SQL route with compact provenance; then delete layer-local duplicate copies only after both Layer 3 and M05 consumers read the shared route. |
...
3. M05 task keys: implement compact batch manifests, then delete per-request keys for completed stages.
4. M05 gate reviews: define compact gate-review contract, then compress/remove verbose diagnostic payloads.
...
7. TE refresh and manifests: preserve TE source rows; compact/delete side products on the regular lifecycle cadence after month-level provenance and fallback reconciliation evidence exist.
8. Layer 3 materialization and M05 source duplication: close through lineage and shared option-source contracts.
