...
DEFAULT_QUARANTINE_DELETE_OUTPUT = Path("storage/lifecycle_execution/quarantine_delete_result.json")
DEFAULT_QUARANTINE_DELETE_SUMMARY_OUTPUT = Path("storage/lifecycle_execution/quarantine_delete_summary.json")
...
def _quarantine_path(record: QuarantineRecheckRecord) -> str:
    return f"storage/quarantine/{record.artifact_id}/{Path(record.physical_path).name}"
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--quarantine-recheck-json", default="storage/quarantine_recheck/quarantine_recheck_evidence.json", help="Quarantine/recheck evidence JSON path.")
